...
__credits__ = ["Nisar Ahmed", "Sousheel Vunnam"]
__license__ = "GPL"
__version__ = "1.0.0"
...
class robberEvasion():
	def __init__(self, copName, robberName):
		...
		self.copSafetyDistribution = scipy.stats.norm(meanCopCost, stdCopCost)
...
def readConfig():
	...
	parentDir = os.path.abspath(os.path.join(curDir, os.pardir))
	parparDir = os.path.abspath(os.path.join(parentDir, os.pardir))
	configLoc = parparDir + '/config/config.yaml'
	...
	for robot in yamled["cop_rob"].keys():
		if yamled["cop_rob"][robot] == 'cop':
			copName = robot
		elif yamled["cop_rob"][robot] == 'rob':
			robName = robot
	return copName, robName
