...
Kochenderfer chapter 6 on individual rooms of the 
cops and robots problem
...
class OnlineSolver():
	...
	def __init__(self):
		modelModule = __import__('D4QuestSoftmaxModel', globals(), locals(), ['ModelSpec'],0); 
		modelClass = modelModule.ModelSpec;
		self.model = modelClass();
		self.model.buildTransition(); 
		self.model.buildObs(gen=False);
		self.model.buildReward(gen=False);
		self.maxMix = 10;   
	...
	def beliefUpdate(self,b,a,o,mod):
		...
		btmp1 = GM(); 
		for j in b.Gs:
			mean = (np.matrix(j.mean) + np.matrix(mod.delA[a])).tolist()[0]; 
			var = (np.matrix(j.var) + np.matrix(mod.delAVar)).tolist(); 
			weight = j.weight; 
			btmp1.addG(Gaussian(mean,var,weight)); 
		btmp = mod.pz.runVBND(btmp1,o); 
		...
		if(len([node for node in PreOrderIter(self.T,filter_=lambda n: n.name==h)]) == 0):
			...
			for a in range(0,self.model.acts):
				if(len([node for node in PreOrderIter(self.T,filter_=lambda n: n.name==newName+str(a))]) == 0):
					...
					tmp = [node for node in PreOrderIter(self.T,filter_=lambda n: n.name==newName+str(a))][0]; 
	...
	def generate(self,s,a):
		...
		ztrial = [0]*self.model.pz.size; 
		for i in range(0,self.model.pz.size):
			ztrial[i] = self.model.pz.pointEvalND(i,sprime); 
...
def testMCTS4D():
	a = OnlineSolver(); 
	b = GM();
	b.addG(Gaussian([3,2,2,0],(np.identity(4)*1).tolist(),1));
	action = a.MCTS(b,d=2); 
	...
	for count in range(0,trails):
		...
		for step in range(0,trailLength):
			...
			for i in range(0,len(x)):
				x[i] = max(x[i],a.bounds[i][0]); 
				x[i] = min(x[i],a.bounds[i][1]); 


			ztrial = [0]*a.model.pz.size; 
			for i in range(0,a.model.pz.size):
				ztrial[i] = a.model.pz.pointEvalND(i,x); 
			z = ztrial.index(max(ztrial)); 
