...
class PBVISolver():
	...
	def __init__(self):
		modelModule = __import__('roomsProblemSpec', globals(), locals(), ['ModelSpec'],0); 
				...
				for o in range(0,self.model.obs):
					for s in range(0,self.model.N):
						for sprime in range(0,self.model.N):
							als1[g][a][o][s] += self.model.pz[o][a][sprime]*self.model.px[a][s][sprime]*self.Gamma[g][sprime]; 
		return als1
	...
	def beliefUpdate(self,bel,a,o):
		...
		for i in range(0,self.model.N):
			...
			belNew[i] = self.model.pz[o][a][i]*belBar[i];
			suma+=belNew[i];  
		#normalize
		for i in range(0,self.model.N):
...
def compareToMDP(ans):
	...
	for i in range(0,ans.model.N):
		...
		print(grid[0,i]//6,grid[0,i]%6); 
	plt.xlim([0,ans.model.N-1]); 
